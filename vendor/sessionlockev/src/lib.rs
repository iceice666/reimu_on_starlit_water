...
pub enum SessionLockEventError {
    ...
    #[error("session lock was refused; another lockscreen may already be running")]
    LockRefused,
    #[error("wayland compositor finished the session lock unexpectedly")]
    LockFinished,
}
...
impl<T> Dispatch<ext_session_lock_v1::ExtSessionLockV1, ()> for WindowState<T> {
    fn event(
        state: &mut Self,
        ...
        match event {
            ...
            ext_session_lock_v1::Event::Finished => {
                ...
                state.lock_error = Some(if state.lock_confirmed {
                    SessionLockEventError::LockFinished
                } else {
                    SessionLockEventError::LockRefused
                });
            }
            _ => {}
        }
...
impl<T: 'static> WindowState<T> {
            ...
            + 'static,
    {
                    ...
                    _ => {
                        ...
                        match event_handler(
                            ...
                            *index_message,
                            ...
                            ReturnData::RequestUnlockAndExist => {
                                window_state.unlock_requested = true;
        ...
        loop {
            ...
            if let Some(error) = window_state.lock_error.take() {
                return Err(error);
            }
